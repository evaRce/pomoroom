defmodule Pomoroom.ChatRoom.ChatServer do
  ...
  def get_messages(chat_id, limit \\ :all, joined_at \\ nil) do
    GenServer.call(via_tuple(chat_id), {:get_messages, limit, joined_at})
  end
  ...
  def get_messages_before(
    ...
    GenServer.call(
      via_tuple(chat_id),
      {:get_messages_before, before_inserted_at, limit, before_db_id, joined_at}
    )
  end
  ...
  def handle_call({:get_messages, :all, joined_at}, _from, state) do
    if joined_at != nil do
      {:ok, messages_from_db} = Messages.get_chat_messages(state.chat_id, :all, joined_at)
      {:reply, messages_from_db, state}
    else
      if state.first_load do
        {:ok, messages_from_db} = Messages.get_chat_messages(state.chat_id, :all, nil)
  ...
  def handle_call({:get_messages, limit, joined_at}, _from, state) when is_integer(limit) do
    if joined_at != nil do
      {:ok, limited_messages_from_db} =
        Messages.get_chat_messages(state.chat_id, limit, joined_at)

      {:reply, limited_messages_from_db, state}
    else
      if state.first_load do
        {:ok, limited_messages_from_db} = Messages.get_chat_messages(state.chat_id, limit, nil)
        ...
        if length(state.messages) < limit do
          ...
          limited_messages =
            state.messages |> Enum.reverse() |> Enum.take(limit) |> Enum.reverse()
  ...
  def handle_call(
    ...
    {:ok, older_messages} =
      Messages.get_chat_messages_before(
        state.chat_id,
        before_inserted_at,
        limit,
        before_db_id,
